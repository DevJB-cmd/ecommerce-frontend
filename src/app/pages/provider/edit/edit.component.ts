...
export class ProviderEditComponent implements OnInit {
  ...
  submit(){ if(this.form.invalid) return; const v = this.form.value; this.loading = true; this.s.update(v.id, v).subscribe({ next: ()=> { this.loading = false; alert('Enregistré'); }, error: ()=> { this.loading = false; this.error='Erreur' } }); }
}
