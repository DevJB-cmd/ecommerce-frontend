...
export class CategoryEditComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private s: CategoryService, private route: ActivatedRoute, private router: Router){ this.form = this.fb.group({ name:['', [Validators.required]], description: [''] }); }
  ...
  submit(){
    ...
    this.loading=true;
    this.s.update(this.id, this.form.value as Category).subscribe({ next: ()=> { this.message='Modifié'; this.loading=false; this.router.navigate(['/admin','categories']); }, error: ()=> { this.error='Erreur'; this.loading=false } });
  }
}
